...
int MechaCommandExecuteList(MechaCommandTxHandler_t transmit, MechaCommandRxHandler_t receive)
{
    ...
    for (i = 0, task = tasks; i < TaskCount; i++, task++)
    {
        ...
        {
            if (result == -EPIPE)
                PlatShowEMessage("%02d. %x%s %s: 101 - rx-Command timed out\n", task->id, task->command, task->args, task->label);
            break;
        }
...
int MechaDefaultHandleRes2(MechaTask_t *task, const char *result, short int len)
{
    if (!pstricmp(result, "2A0"))
        PlatShowEMessage("%02d. %x%s %s: 2A0 - Rx-command error, %s\n", task->id, task->command, task->args, task->label);
    else if (!pstricmp(result, "2A1"))
        PlatShowEMessage("%02d. %x%s %s: 2A1 - Rx-command argument count error\n", task->id, task->command, task->args, task->label);
    else if (!pstricmp(result, "2A2"))
        PlatShowEMessage("%02d. %x%s %s: 2A2 - Rx-command argument range error\n", task->id, task->command, task->args, task->label);
    else
        PlatShowEMessage("%02d. %x%s %s: Unrecognized response: %s\n", task->id, task->command, task->args, task->label, result);
        ...
        case '1': //Rx-NGErr
            switch (task->tag)
            {
                case MECHA_CMD_TAG_INIT_CHECKSUM_CHK:
                    return MechaCmdInitRxChecksumChkHandler(result, len);
                default:
                    PlatShowEMessage("%02d. %x%s: Rx-command error: %s\n", task->id, task->command, task->args, task->label, result);
                    return 1;
            }
            break;
        case '2': //Rx-NGBadCmd
            switch (task->tag)
            {
                ...
                default:
                    if (!pstricmp(result, "2A0"))
                        PlatShowEMessage("%02d. %x%s: 2A0 - Rx-command error\n", task->id, task->command, task->args);
                    else if (!pstricmp(result, "2A1"))
                        PlatShowEMessage("%02d. %x%s: 2A1 - Rx-command argument count error\n", task->id, task->command, task->args, task->label);
                    else if (!pstricmp(result, "2A2"))
                        PlatShowEMessage("%02d. %x%s: 2A2 - Rx-command argument range error\n", task->id, task->command, task->args, task->label);
                    else
                        PlatShowEMessage("%02d. %x%s: Unrecognized response: %s\n", task->id, task->command, task->args, task->label, result);
                    return 1;
            }
        default:
            PlatShowEMessage("%02d. %x%s: Unrecognized response: %s\n", task->id, task->command, task->args, task->label, result);
    ...
    switch (ConType)
    {
        ...
        case MECHA_TYPE_40: // better check cxd aka M Renewal Date
            if (!pstrincmp(MechaName, "000005", 6) || !pstrincmp(MechaName, "000105", 6))
                ...
                return "CXR706080-104GG/-106GG";
            else if (!pstrincmp(MechaName, "000A05", 6) || !pstrincmp(MechaName, "000B05", 6))
                return "CXR706080-702GG";
            else if (!pstrincmp(MechaName, "000C05", 6) || !pstrincmp(MechaName, "000D05", 6))
                return "CXR706080-105GG";
            else if (!pstrincmp(MechaName, "000E05", 6) || !pstrincmp(MechaName, "000F05", 6))
