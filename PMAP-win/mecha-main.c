...
static INT_PTR CALLBACK MechaDlg(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    INT_PTR result;
...
void MenuMECHA(void)
{
    ...
    if (IsOutdatedBCModel())
    {
        MessageBox(g_mainWin, L"B/C-chassis: EEPROM update required.\n",
                   L"EEPROM out-of-date",
                   MB_ICONWARNING | MB_OK);
        return;
    }

    if (MessageBox(g_mainWin,
                   L"This tool allows you to re-calibrate and test the mechanics (skew) of the CD/DVD drive.\n"
                   ...
                   L"\nContinue with MECHA adjustment?",
                   L"Mechanics (skew) Adjustment",
                   MB_YESNO | MB_ICONWARNING) == IDYES)
    {
        DialogBox(g_hInstance, MAKEINTRESOURCE(IDD_DIALOG_MECHA_ADJ), g_mainWin, &MechaDlg);
    }
}
